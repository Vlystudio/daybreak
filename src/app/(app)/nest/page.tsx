    ...
    <div className="mx-auto w-full max-w-2xl space-y-5">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            ...
            <Link href="/nest/gallery" className="text-primary font-medium hover:underline">
              See all 60 →
            </Link>
          </p>
        </div>
        <div className="bg-honey-soft rounded-full px-4 py-2 text-right">
          <p className="text-xl font-semibold text-[#5a3d1a] tabular-nums">{game.seeds} 🌱</p>
      ...
      <Card>
        <CardHeader className="flex-row items-center justify-between space-y-0 pb-2">
          ...
          <span className="text-sage text-sm font-medium">+{game.earnedToday} earned</span>
        </CardHeader>
            ...
            <div key={line.label} className="flex items-center justify-between gap-3 text-sm">
              <span
                className={cn(
                  ...
                  line.done ? "text-foreground" : "text-muted-foreground"
                )}
              >
                <span
                  className={cn(
              ...
              <span
                className={cn(
                  "tabular-nums",
                  line.done ? "text-sage font-medium" : "text-muted-foreground"
                )}
              >
                +{line.amount} 🌱
              </span>
      ...
      <ShopPanel
        ...
        activeBird={
          activeBird && activeSpecies
            ? {
                id: activeBird.id,
                name: activeBird.nickname || activeSpecies.name,
                speciesKey: activeSpecies.key,
                happiness: activeBird.happiness,
                accessory: activeBird.accessory,
              }
            : null
        }
      />
