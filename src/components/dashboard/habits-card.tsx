...
export function HabitsCard({ habits }: { habits: HabitStatus[] }) {
  ...
  return (
    <Card className="relative h-full">
      ...
      {burst > 0 && NEST_ENABLED && (
        <div
          key={`seed-${burst}`}
          className="pointer-events-none absolute inset-x-0 top-9 z-10 flex justify-center"
        >
          <span className="seed-pop text-sage text-sm font-bold">+5 🌱</span>
        </div>
      )}
      <CardHeader className="flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="flex items-center gap-2 text-base">
          <Repeat className="text-sage h-4 w-4" aria-hidden />
          Habits
          {habits.length > 0 && (
            <span
              className={cn(
                "rounded-full px-2 py-0.5 text-xs font-medium",
                allDone ? "bg-sage text-white" : "bg-sage-soft text-sage"
              )}
            >
      ...
      <CardContent className="space-y-2.5 pb-6">
        {formMode !== null && (
          <div className="bg-muted/30 space-y-2.5 rounded-2xl border p-3">
            {formMode === "add" && (
              <>
                <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-muted-foreground text-[11px]">or make your own</span>
                  <span className="bg-border h-px flex-1" />
                </div>
              </>
            )}
            ...
            <div className="flex gap-2">
              ...
              <Button size="sm" variant="ghost" onClick={() => setFormMode(null)}>
                Cancel
              </Button>
            </div>
          </div>
        ...
        {habits.length === 0 && formMode === null && (
          <div className="flex flex-col items-center gap-2 py-5 text-center">
            <span className="bg-sage-soft flex h-11 w-11 items-center justify-center rounded-full text-xl">
              🌱
            </span>
            <p className="text-sm font-medium">Start a streak</p>
            <p className="text-muted-foreground max-w-[15rem] text-xs">
              Add a small daily habit — brushing teeth, water, a 10-minute read.
            </p>
            ...
            <div key={h.id} className="group flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-muted-foreground mt-0.5 flex items-center gap-2 text-xs">
                  {h.streak > 0 && (
                    <span className="text-honey inline-flex items-center gap-0.5 font-medium">
                      <Flame className="h-3.5 w-3.5" aria-hidden /> {h.streak}
                    </span>
                  )}
                  <span
                    className="flex items-center gap-1"
                    aria-label={`${h.weekCount} of ${h.target_per_week} this week`}
                  >
                    {Array.from({ length: h.target_per_week }).map((_, i) => (
                      <span
                        ...
                        className={cn(
                          ...
                          i < h.weekCount ? c.fill : "bg-muted-foreground/20"
                        )}
                      />
                    ))}
                  </span>
