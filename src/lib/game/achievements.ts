...
// Metrics that belong to the Nest bird game (seeds currency + bird collection).
// Their badges are hidden while the Nest is disabled (see @/lib/features).
const NEST_METRICS = new Set<keyof AchievementStats>(["seeds", "birds", "species", "photoBirds"]);

export function computeAchievements(stats: AchievementStats): Achievement[] {
  return CATALOG.filter((a) => NEST_ENABLED || !NEST_METRICS.has(a.metric)).map((a) => {
...
export async function gatherAchievementStats(
  ...
  const [habitDone, foods, checkins, reviews, weights, { data: birds }] = await Promise.all([
    ...
    admin
      .from("user_birds")
      .select("species_key, source")
      .eq("user_id", userId)
      .returns<{ species_key: string | null; source: string }[]>(),
  ]);

  const list = birds ?? [];
  const photoBirds = list.filter((b) => b.source === "photo").length;
  const hatchedSpecies = new Set(
    list.filter((b) => b.source !== "photo" && b.species_key).map((b) => b.species_key)
  );
