# Adds OURA_CLIENT_ID / OURA_CLIENT_SECRET to Vercel via the REST API.
# Reads values + token from .env.local. Leaves all other env vars untouched.
# Run from project root:  .\scripts\add-oura-env.ps1
...
# delete any existing copies first (so re-runs update cleanly)
try {
