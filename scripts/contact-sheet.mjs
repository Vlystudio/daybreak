...
const out = process.argv[4] ?? path.join(os.tmpdir(), "bird-contact.png");
...
const composites = [];
const labels = [
  `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}"><rect width="${W}" height="${H}" fill="#eef1ea"/>`,
];
...
for (let i = 0; i < list.length; i++) {
  ...
  if (fs.existsSync(file)) {
    ...
    labels.push(
      `<text x="${cx + cell / 2}" y="${cy + cell / 2}" font-family="sans-serif" font-size="13" fill="#b04a3a" text-anchor="middle">missing</text>`
    );
  }
  labels.push(
    `<text x="${cx + cell / 2}" y="${cy + cell + 15}" font-family="sans-serif" font-size="11" fill="#3a352e" text-anchor="middle">${from + i + 1}. ${b.commonName}</text>`
  );
}
labels.push("</svg>");

const base = await sharp(Buffer.from(labels.join("")))
  .png()
  .toBuffer();
