...
async function side(b, dir) {
  ...
  const res = await client.images.edit({
    ...
    image: imageFile,
    prompt:
      `Redraw the EXACT same ${b.commonName} from the image — identical colours, markings, body shape and the ` +
      `same cozy soft 3D vector illustration style — but in a FULL SIDE PROFILE, its whole body facing directly ` +
      `to the ${dir} (a 90-degree side view, like a field-guide side profile). Keep field marks: ${b.fieldMarks.join(", ")}. ` +
...
for (const b of list) {
  for (const [dir, suffix] of [["left", "ll"], ["right", "rr"]]) {
    ...
    for (let a = 1; a <= 5 && !done; a++) {
      try {
        fs.writeFileSync(dest, await side(b, dir));
