...
The authenticated deletion action queues a durable job, stores only a hash of
the user-visible status capability, globally signs out the account, and gives
the worker a fast first attempt. The cron worker atomically claims retries and
runs administrative steps in this order:

1. Revoke each Google, Oura, and Fitbit grant at the provider endpoint.
2. Remove user-addressed objects from every Supabase Storage bucket.
3. Remove OAuth credentials, push subscriptions, and notification settings.
4. Remove social and household relationships.
5. Remove records whose foreign keys would otherwise be retained or anonymized.
6. Delete the Supabase Auth user and remaining sessions last.

The process is intentionally retryable: deletes are idempotent, missing storage objects are harmless, and the Auth identity remains available if an earlier step fails. The sequence is not one database transaction because it spans Storage, PostgREST, and Auth APIs. If Auth deletion succeeds but the worker loses its response or cannot finalize the receipt, the service-only operational job remains retryable; an explicit Auth `user_not_found` response is idempotent success. A database function then atomically completes the receipt and removes the job/raw user ID. A backup may retain data according to the infrastructure retention policy; it must not be restored selectively as an active user account without legal and security approval.

Transient provider, storage, database, or Auth errors are retried with bounded
backoff. A job becomes visibly blocked after the retry budget; it never discards
the only local provider credential while remote revocation remains unconfirmed.
The UI exposes status through the unguessable capability without exposing a user
identifier. Logs identify only the named step/error class, never the user ID or
payload.
...
| Credentials and messaging         | `oauth_connections`, `notification_settings`, `push_subscriptions`                                                                                                                                                | Google, Oura, and Fitbit grants are revoked remotely before encrypted local credentials are deleted. Transient revocation failure blocks/retries deletion. Local rows are then deleted before Auth; cascades remain a backstop. Provider-side evidence is still required in the staging test. |
...
- If Auth deletion succeeds but receipt finalization fails, leave the operational job in place; the next claim safely repeats the Auth step and atomically finalizes the receipt/job.
